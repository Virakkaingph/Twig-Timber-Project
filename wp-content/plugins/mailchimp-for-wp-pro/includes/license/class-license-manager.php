	...
	abstract class DVK_License_Manager implements iDVK_License_Manager {
		...
		*/
			...
			if( $result ) {
				...
				if( $result->license === 'valid' ) {
					...
					if( $result->license_limit > 0 && ( $result->license_limit - $result->site_count ) <= 3 ) {
						$message .= sprintf( __( '<a href="%s">Did you know you can upgrade your license?</a>', $this->product->get_text_domain() ), $this->product->get_tracking_url( '/licenses/', 'license-nearing-limit-notice' ) );
					...
					if( isset( $result->error ) && $result->error === 'no_activations_left' ) {
						...
						$this->set_notice( sprintf( __( 'You\'ve reached your activation limit. You must <a href="%s">reset</a> or <a href="%s">upgrade your license</a> to use it on this site.', $this->product->get_text_domain() ), 'https://mc4wp.com/kb/resetting-license-activations/', $this->product->get_tracking_url( '/licenses/', 'license-at-limit-notice' ) ), false );
		...
		public function deactivate_license () {
			...
			if( $result ) {
				...
				} elseif( ! empty( $result->expires ) && strtotime( 'now' ) > $result->expires ) {
		...
		*/
		public function get_license_expiry_date() {
			...
			// make sure we're dealing with a time string
			if( ! is_time( $expiry_date ) ) {
		...
		public function set_license_expiry_date( $expiry_date ) {
			$this->set_option( 'expiry_date', strtotime( $expiry_date ) );
		}
