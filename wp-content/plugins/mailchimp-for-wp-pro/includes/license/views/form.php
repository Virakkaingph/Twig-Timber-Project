...
if( '' !== $this->get_license_key() ) {
	if( $this->get_license_status() === 'expired' ) {
		printf( __( 'Your plugin license has expired. You will no longer have access to plugin updates unless you <a href="%s">renew your license</a>.', 'mailchimp-for-wp' ), $product->get_tracking_url( '/checkout/?edd_license_key=' . $this->get_license_key(), 'renewal_link' ) );
	} else {
		...
		if( ! empty( $expiry_date ) ) {
			...
			if ( strtotime( 'now' ) > $expiry_date ) {
				...
				if ( strtotime( '+3 months' ) > $expiry_date ) {
					printf( ' ' . __( '<a href="%s">Renew your license now</a>.', $product->get_text_domain() ), $product->get_tracking_url( '/checkout/?edd_license_key=' . $this->get_license_key(), 'renewal_link' ) );
				}
			}
